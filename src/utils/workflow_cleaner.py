...
class CleanupReport:
    ...
    def __str__(self) -> str:
        ...
        lines.append(f"Total Folders to Delete: {self.folders_deleted} "
...
class WorkflowCleaner:
    ...
    def identify_deletable_folders(
        ...
        logger.info(f"Topic '{topic}': Keeping {len(to_keep)} folders, deleting {len(to_delete)} folders")
        return to_delete
    ...
    def cleanup(
        ...
        """
        logger.info("=" * 60)
        logger.info(f"Starting workflow cleanup (dry_run={dry_run}, topic_filter={topic_filter}, keep_n={keep_n})")
        logger.info("=" * 60)
        ...
        for topic, folders in grouped.items():
            ...
            deletable_checkpoints = self.identify_deletable_folders(topic, topic_checkpoints, keep_n)
            ...
            topic_details[topic] = {
                ...
                'checkpoint_folders': [
                    {'name': f.path.name, 'size_mb': f.size_mb}
                    for f in deletable_checkpoints
                ],
                'output_folders': [
                    {'name': f.path.name, 'size_mb': f.size_mb}
                    for f in deletable_outputs
                ]
            }
