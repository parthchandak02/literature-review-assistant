    ...
    def detect_study_type(self, extracted_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "confidence": 0.0,
                "reasoning": f"Error during detection: {e!s}. Using fallback checklist.",
                "fallback_used": True,
                "error": str(e),
            }
    ...
    def _parse_detection_response(self, response: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if result["checklist"] not in valid_checklists:
                raise ValueError(
                    f"Invalid checklist '{result['checklist']}'. Must be one of: {valid_checklists}"
                )

            # Ensure confidence is float between 0 and 1
            result["confidence"] = float(result["confidence"])
            if not 0 <= result["confidence"] <= 1:
                logger.warning(f"Confidence {result['confidence']} out of range [0,1]. Clipping.")
