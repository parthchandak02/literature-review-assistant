        ...
        try:
            ...
            if json_match:
                ...
                if 'overall_notes' not in summary:
                    summary['overall_notes'] = f"Automated CASP assessment: {yes_count}/{num_questions} criteria met"
                ...
                logger.warning(
                    f"Could not parse CASP response for {study_title}, using fallback"
                )
                return self._get_fallback_assessment(checklist_type, num_questions=get_checklist_info(checklist_type).get('num_questions', 10))
                
        except Exception as e:
            logger.error(f"Error in CASP assessment for {study_title}: {e}")
            logger.debug(f"Response text: {response[:500] if 'response' in locals() else 'No response'}")
            return self._get_fallback_assessment(checklist_type, num_questions=get_checklist_info(checklist_type).get('num_questions', 10))
    ...
    with Progress(
        ...
        for i, study in enumerate(template["studies"], 1):
            ...
            if is_verbose:
                ...
                progress.log(f"  [dim]-> Building CASP assessment prompt...[/dim]")
