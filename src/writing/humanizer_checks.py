...
_PATTERNS: tuple[tuple[str, str, str], ...] = (
    ...
    ("ghost_citation_claim", MEDIUM, r"\b(studies show|research indicates|data suggests|experts agree)\b"),
    ...
    ("em_dash_overuse", HIGH, r"—|\s--\s"),
    (
        "notability_name_drop",
        MEDIUM,
        r"\b(cited in\b[^.]{0,120}\b(times|bbc|guardian|reuters|cnn|forbes|washington post|new york times)\b|active social media presence)\b",
    ),
    (
        "persuasive_authority_trope",
        MEDIUM,
        r"\b(at its core|what really matters|the real question is|the heart of the matter|fundamentally)\b",
    ),
    (
        "signposting_announcement",
        MEDIUM,
        r"\b(let's dive in|let's explore|here's what you need to know|without further ado|let's break this down)\b",
    ),
    (
        "aphorism_formula",
        LOW,
        r"\b(is the language of|becomes a trap|is not a tool but a mirror|the currency of|the architecture of)\b",
    ),
    (
        "conversational_rhetorical_opener",
        MEDIUM,
        r"(?:(?<=[.!?]\s)|^)(?:Honestly\?|Here's the thing|The thing is)\b",
    ),
    ("sycophantic_tone", HIGH, r"\b(great question|you're absolutely right|that's an excellent point)\b"),
    (
        "cutoff_disclaimer",
        HIGH,
        r"\b(as of my last training|I don't have access to real-time data|maintains a low profile|keeps personal details private|prefers to stay out of the spotlight|it is believed that)\b",
    ),
    (
        "chatbot_artifact",
        HIGH,
        r"\b(I'd be happy to help|as an AI|feel free to ask|would you like|want me to|should I continue|let me know if)\b",
    ),
