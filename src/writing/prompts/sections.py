...
def get_abstract_prompt_context(
    ...
    return (
        ...
        "**Results:** (MUST include: exact N included studies from the FACTUAL DATA BLOCK, "
        "primary finding direction with specific outcomes, and certainty level. Do NOT write "
        "a vague process description -- state what was actually found.)\n"
        f"**Conclusions:** ({conclusions_instruction} -- MUST state a substantive finding, "
        "not merely a process description. Do NOT write 'this review synthesized the evidence' "
        "or 'further research is needed' as the entire conclusion.)\n"
        "**Keywords:** (5-7 comma-separated keywords derived from the research question "
        "and review topic above -- use domain-specific terms, NOT generic methodology terms "
        "like 'systematic review' or 'evidence synthesis')\n\n"
        ...
        f"MINIMUM {target_low} words. Do NOT exceed {abstract_limit} words in the body.\n\n"
...
def get_methods_prompt_context(
    ...
    return (
        ...
        "The wording MUST preserve the exact staged process when that appears "
        "in the FACTUAL DATA BLOCK. Do NOT collapse the staged process into one generic sentence. "
        "Use the exact role language from the block. "
        "Use 'independent reviewer' terminology only; avoid reviewer-label adjectives. "
        ...
        "If the FACTUAL DATA BLOCK contains a 'SCREENING TRANSPARENCY' instruction, "
        "include the full priority scoring disclosure paragraph verbatim in this section. "
...
def get_results_prompt_context(
    ...
    return (
        ...
        "When reporting these reasons, state explicitly that each excluded report was assigned "
        "one primary reason category; do NOT describe the reason counts as overlapping article totals.\n"
        ...
        "somewhere in this Results section. After your narrative description, check the "
        "INCLUDED STUDIES list and group any remaining uncited keys by study design, "
        "appending them as citation clusters, for example: "
        "'Randomized controlled trials in this review included [KeyA2021, KeyB2022]. "
        "Developmental and feasibility studies also include [KeyC2019, KeyD2020, KeyE2023].' "
        "METHODOLOGY REFERENCES (Page2021, Cohen1960, Sterne2019, etc.) do NOT need to appear "
        ...
        "block (using the actual outcome domains from the included evidence -- "
        "e.g., primary outcomes, secondary outcomes, safety, implementation factors). "
...
def get_discussion_prompt_context(
    ...
    return (
        ...
        "If the FACTUAL DATA BLOCK contains an 'EFFECT SIZE VARIABILITY' instruction, "
        "follow it EXACTLY: explain WHY effect sizes vary across studies, identify any "
        "temporal, dose-response, or adoption-curve patterns, and connect variability to "
        "the specific contextual factors (setting, population, intervention intensity, "
        "implementation fidelity) identified in the included evidence. "
        "Do NOT write a generic 'findings align with prior work' "
...
def get_conclusion_prompt_context(
    ...
    if grounding is not None and getattr(grounding, "conclusion_hedging_required", False):
        ...
        hedging_rule = (
            ...
            "with caution', 'hypothesis-generating rather than definitive', and "
            "'further high-quality full-text evidence is required'. "
