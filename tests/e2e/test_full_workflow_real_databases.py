    ...
    return {
        ...
        "agents": {
            ...
            "screening_agent": {
                ...
                "goal": "Screen papers for relevance",
...
class TestFullWorkflowRealDatabases:
    ...
    def test_search_phase_real_databases(self, workflow_manager):
        ...
        print(f"  With authors: {papers_with_authors}/{len(papers)} ({papers_with_authors/len(papers)*100:.1f}%)")
        print(f"  With DOI: {papers_with_doi}/{len(papers)} ({papers_with_doi/len(papers)*100:.1f}%)")
    ...
    def test_prisma_counter_updates(self, workflow_manager):
        ...
        workflow_manager.all_papers = papers
        workflow_manager.prisma_counter.set_found(len(papers), workflow_manager._get_database_breakdown())
