...
class TestCountryExtraction:
    ...
    def test_country_extraction_from_affiliations(self, tmp_path):
        ...
        test_cases = [
            (
                ["Department of Health, University of California, San Francisco, CA, USA"],
                "United States",
            ),
            (
    ...
    def test_subject_normalization(self, tmp_path):
        ...
        for keyword, expected_category in test_cases:
            ...
            assert result == expected_category, (
                f"'{keyword}' should map to '{expected_category}', got '{result}'"
            )
    ...
    def test_journal_subject_inference(self, tmp_path):
        ...
        for journal_name, expected_subject in test_cases:
            ...
            assert result == expected_subject, (
                f"'{journal_name}' should infer '{expected_subject}', got '{result}'"
            )
...
class TestQualityVisualizations:
    ...
    def test_risk_of_bias_plot_generation(
        self, chart_generator, sample_risk_of_bias_assessments, tmp_path
    ):
        """Test risk of bias plot generation."""
        output_path = tmp_path / "rob_plot.png"

        result_path = chart_generator.generate_risk_of_bias_plot(
            sample_risk_of_bias_assessments, str(output_path)
        )
    ...
    def test_grade_evidence_profile_generation(
        self, chart_generator, sample_grade_assessments, tmp_path
    ):
        ...
        result_path = chart_generator.generate_grade_evidence_profile(
            sample_grade_assessments, str(output_path)
        )
