"""Unit tests for manuscript quality fixes.

Tests:
1. fulltext_paper_ids: Full Text Retrieved column uses file presence
2. _normalize_date_range: date range normalization in body text
3. build_picos_table date normalization: inclusion criteria text uses authoritative dates
4. assemble_submission_manuscript passes fulltext_paper_ids through the call chain
"""
...
def test_assemble_submission_manuscript_strips_existing_leading_h1_before_prepend(tmp_path: Path) -> None:
    body = "# A Systematic Review: Duplicate Title\n\n## Introduction\n\nBody text.\n"
    result = assemble_submission_manuscript(
        body=body,
        manuscript_path=tmp_path / "ms.md",
        artifacts={},
