...
| `gates.*` | `profile` (strict / warning), `manuscript_contract_mode` (observe / soft / strict), `manuscript_audit_mode` (observe / soft / strict audit verdict), `audit_gate_mode` (advisory / strict workflow outcome), `search_volume_minimum`, `screening_minimum`, `sparse_topic_min`, `sparse_topic_continuation`, `extraction_completeness_threshold`, `extraction_max_empty_rate`, `cost_budget_max` (USD) |
...
Build phases and enhancement checkpoints in strict dependency order. Each phase writes a completion marker to the `checkpoints` table, including `finalize` after `run_summary.json` is written. The canonical phase key strings are fixed -- any mismatch silently breaks resume.
...
Finalize                   -> checkpoint key: "finalize" + writes run_summary.json + registry status
```
...
Validators run:
- IEEE validator: abstract capped at `settings.ieee_export.max_abstract_words` (default 250), references 30-80, all `\cite{}` resolve in `.bib`, no placeholder text
...
- `GET /api/history/costs/aggregates` -- global totals plus day/week/month/workflow/phase/model grouped views across registry-linked runtime DBs (optional `start_ts`, `end_ts`, `include_archived`)
- `GET /api/history/costs/export` -- global CSV reconciliation export across registry-linked runtime DBs (`granularity=day|week|month`, optional `start_ts`, `end_ts`, `include_archived`)
...
| ResultsView | Top-level final audit summary card, LaTeX export trigger, DOCX download, inline manuscript viewer, PRISMA diagram, GRADE summary of findings, PROSPERO downloads, evidence network panel, and collapsible artifact browser (available when run is done) |
| QualityView | Final audit summary first, then export readiness, PRISMA compliance, diagnostics summary, fallback event overview, and RAG diagnostics |
...
| IEEE submission | Phase 8 | IEEEtran.cls; abstract capped at `settings.ieee_export.max_abstract_words` (default 250); 7-10 pages; numbered BibTeX references; PRISMA checklist as supplementary |
...
- Abstract <= `settings.ieee_export.max_abstract_words` (default 250)
