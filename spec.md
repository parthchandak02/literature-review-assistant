...
**Test status:** 319 passed, 2 skipped (`uv run pytest tests/unit tests/integration -q`). Unit tests cover all manuscript quality improvements (test_manuscript_quality.py: 13 tests for include_rq_block, build_compact_study_table, _trim_abstract_to_limit, _build_citation_coverage_patch design grouping), citation pipeline (test_inject_missing_citations.py, test_citation_grounding.py), and landing-page resolver tier ordering (test_landing_page_resolver.py -- must mock _quick_citation_pdf_url for Tier 0.5).
