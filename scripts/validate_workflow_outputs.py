...
class WorkflowOutputValidator:
    ...
    def validate_papers(self, papers_file: Optional[str] = None) -> Tuple[bool, Dict]:
        ...
        try:
            ...
            console.print(
                f"  With titles: {metrics['papers_with_titles']}/{metrics['total_papers']}"
            )
            console.print(
                f"  With abstracts: {metrics['papers_with_abstracts']}/{metrics['total_papers']}"
            )
            console.print(
                f"  With authors: {metrics['papers_with_authors']}/{metrics['total_papers']}"
            )
        ...
        try:
            ...
            if metrics["width"] < 800:
                self.warnings.append(
                    f"PRISMA diagram width should be >= 800px, got {metrics['width']}"
                )

            if metrics["file_size"] < 1000:
                self.warnings.append(
                    f"PRISMA diagram file size seems small: {metrics['file_size']} bytes"
                )

            if metrics["file_size"] > 10 * 1024 * 1024:
                self.warnings.append(
                    f"PRISMA diagram file size seems large: {metrics['file_size']} bytes"
                )
    ...
    def validate_article_sections(self, sections_dir: Optional[str] = None) -> Tuple[bool, Dict]:
        ...
        for section in expected_sections:
            ...
            if section_files:
                ...
                if content_length > 100:
                    ...
                    self.warnings.append(
                        f"Section '{section}' has very little content ({content_length} chars)"
                    )
            else:
        ...
        console.print(
            f"  Sections found: {len(metrics['sections_found'])}/{len(expected_sections)}"
        )
    ...
    def validate_final_report(self, report_file: Optional[str] = None) -> Tuple[bool, Dict]:
        ...
        try:
            ...
            for section in expected_sections:
                ...
                if (
                    section_lower in content_lower
                    or section_lower.replace("# ", "") in content_lower
                ):
                    metrics["sections_found"].append(section)
                else:
                    metrics["sections_missing"].append(section)
            ...
            if metrics["sections_missing"]:
                console.print(
                    f"  [yellow]Missing sections: {', '.join(metrics['sections_missing'])}[/yellow]"
                )
            ...
            try:
                ...
                if prisma_results["missing_items"]:
                    console.print(
                        f"  [yellow]Missing PRISMA items: {len(prisma_results['missing_items'])}[/yellow]"
                    )
            except Exception as e:
            ...
            if metrics["content_length"] < 1000:
                self.warnings.append(
                    f"Final report seems short: {metrics['content_length']} characters"
                )
