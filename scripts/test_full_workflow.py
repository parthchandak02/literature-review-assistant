...
class WorkflowTester:
    ...
    def validate_search_phase(self) -> Tuple[bool, Dict]:
        ...
        try:
            ...
            console.print(
                f"  Titles: {papers_with_titles}/{len(papers)} ({papers_with_titles / len(papers) * 100:.1f}%)"
            )
            console.print(
                f"  Abstracts: {papers_with_abstracts}/{len(papers)} ({papers_with_abstracts / len(papers) * 100:.1f}%)"
            )
            console.print(
                f"  Authors: {papers_with_authors}/{len(papers)} ({papers_with_authors / len(papers) * 100:.1f}%)"
            )
            console.print(
                f"  DOI: {papers_with_doi}/{len(papers)} ({papers_with_doi / len(papers) * 100:.1f}%)"
            )
    ...
    def validate_screening_phase(self) -> Tuple[bool, Dict]:
        ...
        try:
            ...
            metrics = {
                ...
                "excluded_count": len(self.workflow_manager.unique_papers)
                - len(self.workflow_manager.screened_papers),
            }
