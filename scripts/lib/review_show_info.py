...
Usage:
    uv run python scripts/review.py info --workflow-id wf-d042e90e
    uv run python scripts/review.py info --run-dir runs/2026-03-05/.../run_03-46-58AM
    uv run python scripts/review.py info --workflow-id wf-d042e90e --fetch-pdfs
"""
...
async def run_info(
    ...
    if db_path is None or not db_path.exists():
        console.print(
            ...
            f"{'workflow-id=' + workflow_id if workflow_id else 'run-dir=' + str(run_dir)}[/]"
        )
        return 1
    await show_run(db_path, fetch_pdfs=fetch_pdfs, run_root=run_root)
    return 0
