...
Usage:
    uv run python scripts/repair.py re-extract --run-dir <path-to-run-directory>
    uv run python scripts/repair.py re-extract --run-dir <path> --config <path-to-review.yaml>
...
After this script completes, run `uv run python scripts/repair.py finalize --run-dir ...`
to regenerate the manuscript with the improved extraction data.
"""
...
async def main(run_dir: str, config_override: str | None) -> int:
    ...
    if improved:
        console.print(
            "\n[bold]Next step:[/] run "
            "[cyan]uv run python scripts/repair.py finalize --run-dir "
            f"{run_dir}[/] to regenerate the manuscript."
        )
    return 0 if still_failed == 0 else 2


async def run_re_extract(run_dir: str, config_override: str | None = None) -> int:
    return await main(run_dir, config_override)
