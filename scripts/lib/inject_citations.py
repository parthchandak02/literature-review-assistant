...
Usage:
    uv run python scripts/repair.py inject-citations --run-dir <path-to-run-directory>
    uv run python scripts/repair.py inject-citations --workflow-id <workflow-id>

Example:
    uv run python scripts/repair.py inject-citations \\
        --run-dir runs/2026-03-08/wf-0004-.../run_10-41-36PM
    uv run python scripts/repair.py inject-citations --workflow-id wf-0005
...
# Artifact paths relative to the run directory (mirrors maint_finalize).
...
async def main(run_dir_str: str) -> int:
    ...
    figure_paths = [f for f in _FIGURE_NAMES if (run_path / f).exists()]
...
async def run_inject_citations(run_dir: pathlib.Path) -> int:
    return await main(str(run_dir))


def resolve_run_dir(*, run_dir: str | None, workflow_id: str | None) -> pathlib.Path:
    ...
    if registry_db is None:
        raise FileNotFoundError(
            f"Could not find workflows_registry.db to resolve workflow-id '{workflow_id}'. "
    ...
    if row is None:
        raise LookupError(f"workflow_id '{workflow_id}' not found in registry at {registry_db}.")
